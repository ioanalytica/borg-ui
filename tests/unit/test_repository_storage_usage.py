...
def test_store_target_and_rclone_remote():
    assert storage_usage.store_target("rest://borg@k3s01/m3s/repo") == ("", None)
...
def test_cancelled_http_walk_has_ended_its_child_when_cancel_returns(
    ...
    monkeypatch.setattr(
        storage_usage,
        "_http_walk_command",
        lambda: ["sh", "-c", f"echo $$ > {pid_file}; exec sleep 30"],
    )
    started = lambda: pid_file.exists()  # noqa: E731
    t0 = time.monotonic()
    with pytest.raises(storage_usage.Cancelled):
...
def test_ending_a_child_ends_its_descendants_too(monkeypatch, tmp_path):
    ...
    cmd = [
        "sh",
        "-c",
        f"trap '' TERM; sleep 30 & echo $! > {pid_file}; sleep 30; wait",
    ]
    t0 = time.monotonic()
    with pytest.raises(subprocess.TimeoutExpired):
        storage_usage._run(cmd, timeout=0.2)
    assert time.monotonic() - t0 < 10
