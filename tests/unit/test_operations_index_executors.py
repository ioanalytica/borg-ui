...
@pytest.mark.parametrize(
    ...
    [
        ...
        (True, False, "pending"),  # Community: the stage is absent for everyone
...
async def test_run_archive_sync_marks_agent_archives_skipped(
    ...
    """No history run reaches an agent's repository on a plan with the
    feature, so the listing writes the state that run used to write: a new
    archive is `skipped`, not a `pending` that reads as "not yet"; an
    indexed one is left alone. Everywhere else `pending` stays, so a later
    plan or executor change finds it."""
    monkeypatch.setattr(
        index_exec,
        "list_archives_for_repository",
        AsyncMock(return_value=(True, [BORG1_ENTRY], "UTC")),
    ...
    db.add_all([earlier, given_up, other_repo])
    db.flush()
    db.add(
        Archive(
