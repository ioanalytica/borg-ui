...
class TestDashboardOverviewAggregates:
    ...
    def test_rows_of_a_deleted_repository_match_by_id_and_keep_the_path_name(
        ...
        """Without foreign keys enforced (SQLite by default), a deleted
        repository leaves its operations behind: the failure is named by the
        path a row captured, and a later run with the same id resolves it
        ...
        test_db.add_all(
            [
                row("check", "failed", 6, {"repository_path": "/gone/x"}, "old"),
                row("check", "completed", 5, {}),
                row("prune", "failed", 4, {"repository_path": "/gone/x/"}, "bad"),
            ]
        )
        test_db.commit()
