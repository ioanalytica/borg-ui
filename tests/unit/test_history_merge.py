...
@pytest.mark.unit
@pytest.mark.parametrize("has_borg_identity", [False, True])
async def test_legacy_targets_wait_for_a_listing_with_observation_identity(
    db, repo, has_borg_identity
):
    removed = _archive(db, repo, "removed", 1)
    removed_id = removed.id
    ...
    parent.result = {
        "removed_archive_ids": [removed_id],
        **(
            {"removed_archive_borg_ids": {str(removed_id): removed.borg_id}}
            if has_borg_identity
            else {}
        ),
    }
    db.commit()
