...
def bounded_text(lines: list[str]) -> str:
    ...
    leave the credential unrecognisable. A single line beyond the bound is
    not sent at all; its length stands in for it."""
    kept: list[str] = []
    size = 0
    for line in reversed(lines):
        if size + len(line) + (1 if kept else 0) > FAILURE_TAIL_CHARS:
            break
        kept.append(line)
        size += len(line) + (1 if size else 0)
    if not kept and lines:
        return f"[a line of {len(lines[-1])} characters was left out]"
    return "\n".join(reversed(kept))
