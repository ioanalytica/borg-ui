...
def _http_walk(url: str, timeout: float) -> int:
...
def store_target(url: str) -> tuple[str, Optional[str]]:
    ...
    if url.startswith(("http://", "https://")):
        return "http", url
    if url.startswith(("sftp://", "rclone:")):
        return "rclone", url
    if url.startswith("rest://"):
        parts = urlsplit(url)
        return ("", None) if parts.hostname else ("du", parts.path)
    if "://" not in url and not url.startswith(("s3:", "b2:")):
        return "du", url
    return "", None
...
def measure(
    ...
    *,
    ...
    if indexed is not None:
        ...
        return {
            "bytes": size if size > 0 else None,
            "objects": objects,
            "source": SOURCE_BORG2_INDEX if size > 0 else None,
        }
    _check_cancel(should_cancel)
    budget = remaining()
    if budget <= 0:
