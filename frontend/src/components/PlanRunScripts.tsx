...
import type { BackupJob, BackupPlanRun, BackupPlanScriptExecution } from '../types'

// A log target opened from a plan run: either a repository backup job or one of
// the plan's script (hook) executions.
...
// Script executions are exempt from the log-save policy server-side, so their
// logs are viewable whenever they ran (not pending) and the server flagged
// output — or while they are still running.
function canViewScriptLogs(execution: BackupPlanScriptExecution): boolean {
  return (
    execution.status !== 'pending' &&
    Boolean(execution.has_logs || execution.status === 'running')
  )
}
