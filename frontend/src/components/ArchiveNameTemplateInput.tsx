...
const ArchiveNameTemplateInput: React.FC<ArchiveNameTemplateInputProps> = ({
  ...
  // the backend and borg's own expansion); {utcnow} renders in UTC.
  const previewName = useMemo(() => {
    ...
    // Millisecond precision matches the backend's bare-placeholder expansion.
    const localIso = `${date}T${pad(now.getHours())}-${pad(now.getMinutes())}-${pad(now.getSeconds())}-${pad3(now.getMilliseconds())}`
    const utcIso = now.toISOString().replace(/[:.]/g, '-').slice(0, -1)
