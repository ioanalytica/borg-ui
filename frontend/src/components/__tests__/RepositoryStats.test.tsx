...
describe('RepositoryStats', () => {
  ...
  describe('the values', () => {
    ...
    it('withholds the ratio when its two sides are different measurements', () => {
      ...
      // a later compact can write the size without reporting a source
      // data size, so both carry the label while describing two states
      const sameLabel = {
        ...mixed,
        size_source: 'compact_stats' as const,
        measured_at: '2026-09-18T04:48:36.000Z',
      }
      const [, , labelSaved] = repositoryStatItems(t, {
        storage: sameLabel,
        variant: 'grid',
      }).slice(1)
      expect(labelSaved.state).not.toBe('value')

      // one compact reported both: the figure is not older than the size
      const sameRun = { ...sameLabel, measured_at: '2026-09-11T04:48:30.000Z' }
      const [, , sameSaved] = repositoryStatItems(t, { storage: sameRun, variant: 'grid' }).slice(1)
      expect(sameSaved.state).toBe('value')
