...
export function deriveTrack(
  ...
  // The server decides which kinds take a lane; the track only needs to
  // know which running rows could plausibly be that one, to tell "the lane
  // is taken" apart from "the index workers are busy". Backup and the
  // maintenance kinds are exclusive, and so is the file-history index,
  // which is filed under `index`. A drift against the server's table costs
