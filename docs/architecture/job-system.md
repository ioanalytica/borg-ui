...
- The repository status (`GET /repositories/{id}/status`, also served as
  ...
  is the newest `archive_sync` that reported removed archives unless a
  prune run through Borg UI is newer; check and compact keep their job
  rows. Overdue is judged per series against its own cadence (the
  ...
  enabled, scheduled and linked through an enabled association); it is
