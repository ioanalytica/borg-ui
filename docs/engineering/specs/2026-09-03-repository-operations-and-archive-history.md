...
whatever created it; a failed Borg UI attempt newer than that archive shows
as the failure; with no archives the newest job row stands. Prune is the
newest `archive_sync` that reported removed archives, unless a prune run
...
counts only when it is enabled, scheduled (not manual-only) and its
association with the repository is enabled; a legacy scheduled job counts
by the scheduler's own target precedence (association rows, else the
direct repository, else the repository path). Otherwise `overdue` is
