    ...
    async def execute_restore_check(self, job_id: int, repository_id: int):
        ...
        try:
            ...
            if job_id in self.cancelled_jobs:
                ...
                # verified nothing, whatever its last extract returned. The
                # executor turns a failed row it cancelled into `cancelled`.
                job.status = "failed"
                job.progress = 100
                job.completed_at = datetime.utcnow()
    ...
    async def _await_agent_operation(
        ...
        while True:
            ...
            if never_claimed or went_silent:
                ...
                elif agent_job.status not in TERMINAL_AGENT_STATUSES:
