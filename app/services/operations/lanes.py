...
def write_maintenance_running(db: Session, repository_id: int) -> bool:
    """True while prune, compact, delete or wipe is pending or running on
    the repository, as an operation, a legacy job row, or a backup job in
    its maintenance phase. Unlike a running backup, these are not something
    ...
    if (
        ...
        .filter(
            BackupJob.repository_id == repository_id,
            BackupJob.status.in_(_LEGACY_MAINTENANCE_BACKUP_STATUSES),
        )
        .first()
    ):
        return True
