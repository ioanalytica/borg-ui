...
def removed_archive_targets_from_dependency(
    ...
    return [(int(i), identities.get(str(i)), observations.get(str(i))) for i in ids]
...
async def run_history_merge(ctx) -> Outcome:
    ...
    for position, (archive_id, borg_id, last_seen_at) in enumerate(targets):
        if ctx.cancelled():
            break
        ...
        if (
            ...
            # Results without an observation identity cannot safely delete:
