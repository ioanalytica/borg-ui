...
def archive_starts(db: Session, repository_id: int) -> list[datetime]:
    return [
        row[0]
        for row in db.query(Archive.start)
...
def latest_removal(db: Session, repository_id: int) -> Optional[datetime]:
    """When archives were last seen to disappear: the newest archive_sync
    whose result lists removed archives. An upper bound, since the removal
    happened between that listing and the one before."""
...
def backup_cell(db: Session, repository: Repository, spec: dict) -> CellStatus:
    jobs = job_evidence(db, repository.id, "backup", spec)
    starts = archive_starts(db, repository.id)
    if not starts:
        return jobs
    cell = CellStatus(
        cell="backup",
...
def _plan_runs(db: Session, repository_id: int, column: str) -> bool:
    ...
    dispatcher requires schedule_enabled too), with this repository's
    association enabled, and the step switched on."""
    return (
...
def repository_status(
    ...
    for name, spec in CELLS:
        if name == "mirror" and not mirror_applies:
            continue
        if name == "backup":
            cell = backup_cell(db, repository, spec)
        elif name == "prune":
