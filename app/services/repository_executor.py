...
def agent_timezone_for_repository(db: Session, repository: Repository) -> Optional[str]:
    ...
    Borg emits archive times in the local time of the machine that created the
    archive; for an agent-executed repository that machine is the agent, so its
    reported zone is the one to interpret those times with. None for non-agent
    repositories and for agents that never reported a zone.
    """
