...
def safe_url(url: str) -> str:
    """A repository URL for logs: the password, if any, replaced by `***`.
    ...
    try:
        ...
        return parts._replace(netloc=f"{username}:***@{hostport}").geturl()
    except ValueError:
...
def borg2_interpreter(borg2_binary: str, env: Optional[dict] = None) -> Optional[str]:
    ...
    candidates = []
    pointed = (env or os.environ).get("BORG2_BINARY")
    if pointed:
...
def _http_walk(base_url: str, timeout: int) -> Optional[int]:
    ...
    def walk(name: str) -> None:
        ...
        response = requests.get(
            f"{root}/{name}/" if name else f"{root}/",
            auth=auth,
            headers=headers,
            timeout=timeout,
        )
...
async def http_storage_used(url: str, *, timeout: int = 60) -> Optional[int]:
    """`http(s)://` borgstore REST servers list directories with sizes."""
    try:
        value = await asyncio.to_thread(_http_walk, url, timeout)
...
def store_target(repository_path: str) -> tuple[str, Optional[str]]:
    ...
    if repository_path.startswith(("sftp://", "rclone:")):
        return "rclone", repository_path
    if repository_path.startswith("rest://"):
    ...
    if "://" not in repository_path and not repository_path.startswith(("s3:", "b2:")):
        return "du", repository_path
    return "", None
...
async def storage_used(
    ...
    *,
    ...
    if tool == "http":
        return await http_storage_used(target, timeout=min(timeout, 60))
    if tool == "rclone":
    ...
    *,
    ...
    if (repository.borg_version or 1) != 2:
        ...
        try:
            ...
            return SizeResult()
        size = (payload.get("cache") or {}).get("stats", {}).get("unique_csize")
        return SizeResult(
    ...
    used = await storage_used(repository.path, key_file=temp_key_file, env=env)
    return SizeResult(
        bytes=used,
