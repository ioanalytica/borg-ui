...
def _complete_finished_operation_log(
    db: Session, agent_job_id: int, sequence: int, message: str
) -> None:
    ...
    try:
        ...
        if not path or not os.path.isfile(path):
            # never written, or removed by log retention
            return
        out_of_order = (
        ...
        if out_of_order is None:
            ...
            with open(path, "a", encoding="utf-8") as handle:
                handle.write(separator + message)
            return
        tmp_path = f"{path}.tmp"
        try:
            with open(tmp_path, "w", encoding="utf-8") as handle:
                handle.write(_collect_agent_logs(job, db))
...
async def _handle_agent_session_message(
    ...
    if message_type == "log":
        ...
        if job:
            ...
            db.commit()
            if appended:
                _complete_finished_operation_log(db, agent_job_id, sequence, text)
        return
...
async def upload_job_log(
    ...
    job.updated_at = _now_utc()
    db.commit()
    _complete_finished_operation_log(db, job_id, payload.sequence, payload.message)
