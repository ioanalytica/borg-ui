...
from app.services.operations.backup_facade import IN_CHUNK, recent_backup_jobs
...
def _params_by_id(db: Session, ids: list) -> dict:
    """`operations.params` of the given rows, in chunks that stay under the
    SQLite variable limit."""
    params = {}
    for start in range(0, len(ids), IN_CHUNK):
        chunk = ids[start : start + IN_CHUNK]
        for row_id, row_params in db.query(Operation.id, Operation.params).filter(
            Operation.id.in_(chunk)
        ):
            params[row_id] = row_params or {}
    return params
...
def current_failures(
    db: Session, rows, repo_id_map: dict, repo_name_map: dict, repo_path_map: dict
) -> list:
    ...
    without an id captured; a row with neither resolves nothing and is
    resolved by nothing."""
    statuses = {
    ...
    if not failed:
        return []
    # a row whose repository the map does not know (no id, or a repository
    # since deleted) names it in `params`; only the rows that take part in
    # the comparison need it
    params_ids = [
        row.id
        for row in rows
        if row.repository_id not in repo_id_map
        and (statuses[row.id] == "failed" or statuses[row.id] in RESOLVING_STATUSES)
    ]
    params = _params_by_id(db, params_ids)
...
def get_dashboard_overview(
    ...
    try:
        ...
        failures = current_failures(
            db, recent_rows, repo_id_map, repo_name_map, repo_path_map
        )
